...
class CrossModalRepr_wWrong():
	...
	def build_model(self):
		...
		image_wrong = tf.placeholder( tf.float32, [config.batch_size, config.image_size, config.image_size, 3] )
		...
		image_resized_wrong = resize_image_for_inception( image_wrong )
		...
		inception_output_wrong = image_embedding.inception_v3( image_resized_wrong, trainable=False, is_training=True, reuse=True )
		with tf.variable_scope('image_embedding') as image_emb_scope:
			image_emb_scope.reuse_variables()
			image_embeddings_wrong = tf.contrib.layers.fully_connected( inputs = inception_output_wrong,
																	num_outputs = config.embedding_size,
																	activation_fn = None,
																	weights_initializer = self.initializer,
																	biases_initializer = None,
		...
		with tf.variable_scope('lstm', initializer=self.initializer) as lstm_scope:
			...
			_, initial_state_image_wrong = lstm_cell_withDropout( image_embeddings_wrong, zero_state )
			...
			lstm_outputs_image, _ = tf.nn.dynamic_rnn( cell=lstm_cell_withDropout,
														...
														dtype = tf.float32,
														scope = lstm_scope )
			lstm_outputs_image_wrong, _ = tf.nn.dynamic_rnn( cell=lstm_cell_withDropout,
														inputs = input_embeddings,
														sequence_length = text_lengths,
														initial_state = initial_state_image_wrong,
														dtype = tf.float32,
														scope = lstm_scope )
...
#		tf.losses.add_loss( batch_loss_image )
		...
		with tf.variable_scope('d_lstm', initializer=self.initializer) as lstm_scope:
			...
			_, d_initial_state_image_wrong = d_lstm_cell( image_embeddings_wrong, zero_state )
			...
			d_outputs_image, _ = tf.nn.dynamic_rnn( cell = d_lstm_cell,
													...
													dtype = tf.float32,
													scope = lstm_scope )
			d_outputs_image_wrong, _ = tf.nn.dynamic_rnn( cell = d_lstm_cell,
													inputs = lstm_outputs_image_wrong,
													sequence_length = text_lengths,
													initial_state = d_initial_state_image_wrong,
													dtype = tf.float32,
													scope = lstm_scope )
		# gather last output
		...
		d_last_output_text = tf.gather_nd( d_outputs_text, gather_idx )
		d_last_output_image = tf.gather_nd( d_outputs_image, gather_idx )
		d_last_output_image_wrong = tf.gather_nd( d_outputs_image_wrong, gather_idx )

		# FC discriminator : text-vs-image
		with tf.variable_scope('d_logits') as logits_scope:
			d_logits_text = tf.contrib.layers.fully_connected( inputs = d_last_output_text,
														num_outputs = 1,
														activation_fn = None,
														weights_initializer = self.initializer,
														scope = logits_scope )
			logits_scope.reuse_variables()
			d_logits_image = tf.contrib.layers.fully_connected( inputs = d_last_output_image,
														num_outputs = 1,
														activation_fn = None,
														weights_initializer = self.initializer,
														scope = logits_scope )
			d_logits_image_wrong = tf.contrib.layers.fully_connected( inputs = d_last_output_image_wrong,
														num_outputs = 1,
														activation_fn = None,
														weights_initializer = self.initializer,
														scope = logits_scope )
		...
		d_loss_image_wrong = tf.reduce_mean( tf.nn.sigmoid_cross_entropy_with_logits( name = 'd_loss_image',
										logits = d_logits_image_wrong, labels = tf.zeros_like(d_logits_image_wrong) ) )
		d_loss = d_loss_text + d_loss_image + d_loss_image_wrong

		d_acc_text = tf.reduce_mean( tf.sigmoid(d_logits_text) )
		d_acc_image = tf.reduce_mean( 1-tf.sigmoid(d_logits_image) )
		d_acc_image_wrong = tf.reduce_mean( 1-tf.sigmoid(d_logits_image_wrong) )
...
#		summary_g_merge = tf.summary.merge( [summary_g_loss, summary_g_loss_text, summary_g_loss_image] )
		summary_g_merge = tf.summary.merge( [summary_g_loss, summary_g_loss_image] )

		self.input_tensors = {
			'image' : image,
			'image_wrong' : image_wrong,
	...
	def train(self):
		...
		for i in range(config.nEpochs):
			...
			while batch_no*config.batch_size < self.loaded_data['data_length']:
				caption_vectors, captions, captions_shifted, mask, images, images_wrong = self.get_training_batch(batch_no)
				...
				_, D_loss, summary_D = self.sess.run( [optim_D, loss['D'], summary['D']],
									feed_dict={ input_tensors['text_feature']:caption_vectors,
												...
												input_tensors['input_mask']:mask,
												input_tensors['image']:images,
												input_tensors['image_wrong']:images_wrong
												} )
				...
				errD_image_wrong = accuracy['D_image_wrong'].eval(
									feed_dict={	input_tensors['input_text']:captions,
												...
												input_tensors['input_mask']:mask,
												input_tensors['image_wrong']:images_wrong } )
				lossnames_to_print = ['NLL', 'D', 'errD_text', 'errD_img', 'errD_wrong', 'G']
				losses_to_print = [NLL_loss, D_loss, errD_text, errD_image, errD_image_wrong, G_loss]
				...
				if (batch_no % config.save_every_batch) == 0:
					...
					f_valid_txt.write( 'GT) ' + ' '.join([ self.vocab.id_to_word(int(w)) for w in captions[0,1:] ] ) + '\n' )
					f_valid_txt.flush()
	...
	def get_training_batch(self, batch_no):
		...
		images_wrong = np.zeros( (batch_size, image_size, image_size, 3) )
		...
		for i in range(batch_no * batch_size, (batch_no+1) * batch_size ):
			...
			images[cnt,:,:,:] = image
			images_wrong[(cnt+1)%batch_size,:,:,:] = image
		...
		return text_features, mat_captions, mat_captions_shifted, mat_captions_mask, images, images_wrong
