...
def discriminator(image, options, reuse=False, name="discriminator"):
	...
	with tf.variable_scope(name):
		...
		h1 = lrelu(d_bn1(conv2d(h0, options.df_dim*2, name='d_h1_conv')))
		# h1 is (64 x 64 x self.df_dim*2)
		h2 = lrelu(d_bn2(conv2d(h1, options.df_dim*4, name='d_h2_conv')))
		# h2 is (32x 32 x self.df_dim*4)
		h3 = lrelu(d_bn3(conv2d(h2, options.df_dim*8, d_h=1,d_w=1, name='d_h3_conv')))
...
def generator_resnet(image, options, reuse=False, name="generator"):
	...
	with tf.variable_scope(name):
		...
		def residule_block(x, dim, ks=3, s=1, name='res'):
			resBN1 = batch_norm(name=name+'_bn1')
			resBN2 = batch_norm(name=name+'_bn2')
			p = int((ks - 1) / 2)
			y = tf.pad(x, [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
			y = resBN1(conv2d(y, dim, ks,ks, s,s, padding='VALID', name=name+'_c1'))
			y = tf.pad(tf.nn.relu(y), [[0, 0], [p, p], [p, p], [0, 0]], "REFLECT")
			y = resBN2(conv2d(y, dim, ks,ks, s,s, padding='VALID', name=name+'_c2'))
			return y + x

		g_e1_bn = batch_norm(name='g_e1_bn')
		g_e2_bn = batch_norm(name='g_e2_bn')
		g_e3_bn = batch_norm(name='g_e3_bn')
		g_d1_bn = batch_norm(name='g_d1_bn')
		g_d2_bn = batch_norm(name='g_d2_bn')
		g_pred_bn = batch_norm(name='g_pred_bn')
		...
		c1 = tf.nn.relu(g_e1_bn(conv2d(c0, options.gf_dim, 7,7, 1,1, padding='VALID', name='g_e1_c')))
		c2 = tf.nn.relu(g_e2_bn(conv2d(c1, options.gf_dim*2, 3,3, 2,2, name='g_e2_c')))
		c3 = tf.nn.relu(g_e3_bn(conv2d(c2, options.gf_dim*4, 3,3, 2,2, name='g_e3_c')))
		...
		d1 = deconv2d(r9, [options.batch_size, s2, s2, options.gf_dim*2], 3,3, 2,2, name='g_d1_dc')
		d1 = tf.nn.relu(g_d1_bn(d1))
		d2 = deconv2d(d1, [options.batch_size, s, s, options.gf_dim], 3,3, 2,2, name='g_d2_dc')
		d2 = tf.nn.relu(g_d2_bn(d2))
		d2 = tf.pad(d2, [[0, 0], [3, 3], [3, 3], [0, 0]], "REFLECT")
		pred = conv2d(d2, options.output_c_dim, 7,7, 1,1, padding='VALID', name='g_pred_c')
		pred = tf.nn.tanh(g_pred_bn(pred))
